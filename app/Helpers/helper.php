...
{
    if (auth()->check()) {
        if ($menu->data('role') == null && auth()->user()->hasRole(['admin','super_admin'])) {
            return true;
        }
...
function getMultipleMediaSetting($key, $collection = 'subscription_image')
{
    $setting = Setting::where('key', $key)->first();

    if ($setting && method_exists($setting, 'getMedia')) {
        return $setting->getMedia($collection)->map(function ($media) {
            return $media->getUrl(); // Get only the URLs
        })->toArray();
    }
...
{
    $type = [
        '0' => __('message.track_cycle'),
        // '1' => __('message.get_pragnancy'),
        '1' => __('message.track_pragnancy')
    ];
...
{
    $type = [
        '0' => __('message.story_view'),
        '1' => __('message.video'),
        '2' => __('message.categorie'),
        '3' => __('message.video_course'),
        '4' => __('message.blog_course'),
        '5' => __('message.podcast'),
        '6' => __('message.article'),
        // '3' => __('message.image_video')
    ];
...
function getArticleType(){
    $type = [
        '0' => __('message.normal'),
        '1' => __('message.menstural_phase'),
        '2' => __('message.folliculer_phase'),
        '3' => __('message.ovulation_phase'),
        '4' => __('message.luteal_phase'),
        '5' => __('message.late_period'),
        '6' => __('message.pregnancy')
    ];
    return $type;
}
...
function restoreForceDeleteData($model = null, $action_type = null, $message_name = null)
{
    $model = $model;
    $value['status'] = request()->ajax() ? false : 'error';
    $value['message'] = __('message.not_found_entry', ['name' => __('message.' . $message_name)]);

    if ( env('APP_DEMO') && in_array($action_type, ['forcedelete']) ) {
        $value['message'] = __('message.demo_permission_denied');        
        return $value;
    }
    ...
    if ($model != null) {
        switch ($action_type) {
            case 'delete':
                $model->delete();
                $value['status'] = request()->ajax() ? true : 'success';
                $value['message'] = __('message.delete_form', ['form' => __('message.' . $message_name)]);
                break;
            case 'restore':
                $model->restore();
                $value['status'] = request()->ajax() ? true : 'success';
                $value['message'] = __('message.msg_restored', ['name' => __('message.' . $message_name)]);
                break;
            case 'forcedelete':
                $model->forceDelete();
                $value['status'] = request()->ajax() ? true : 'success';
                $value['message'] = __('message.msg_force_delete', ['name' => __('message.' . $message_name)]);
                break;
            default:
                return $value;
                break;
