...
<div class="px-2 py-2">
    <h6 class="text-sm text-muted m-0"><span class="notification_count">{{  __('message.you_have_unread_notification',['number' => $all_unread_count ]) }}</span>
        @if($all_unread_count > 0 )
            <a href="javascript:void(0);" data-type="markas_read" class="notifyList float-right" ><span>{{ __('message.mark_all_as_read') }}</span></a>
        @endif
    </h6>
</div>
...
@else
    <a href="javascript:void(0);" class="sub-card">
        <div class="media align-items-center">
            <div class="media-body ml-3">
