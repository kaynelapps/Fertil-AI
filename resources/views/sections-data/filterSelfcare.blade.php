...
<div class="modal-dialog modal-fullscreen-right" role="document">
    ...
    {{ html()->form('GET', route('section-data-main.index'))->id('filter-form')->open() }}
        <div class="modal-body">
            ...
            <div class="form-group col-md-12">
                ...
                {{ html()->select('category_id', $params['category_id'] ? [$params['category_id'] => $params['category_name']] : [], old('category_id', $params['category_id']))->class('select2js form-group')->id('category_id')->attribute('data-placeholder', __('message.select_name', ['select' => __('message.category')]))->attribute('data-ajax--url', route('ajax-list', ['type' => 'get_category']))->attribute('data-allow-clear', 'true') }}

            </div>
        </div>
        <div class="modal-footer">
            {{ html()->submit(__('message.apply_filter'))->class('btn btn-primary btn-sm mr-2')->id('btn_submit') }}
            <a href="{{ route('section-data-main.index') }}" class="btn btn-orange btn-sm" title="{{ __('message.reset_filter') }}">
