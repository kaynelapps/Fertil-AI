...
<x-master-layout :assets="$assets ?? []">
   <div>
       <?php $id = $id ?? null;?>
        @if(isset($id))
            {{ html()->modelForm($data, 'PATCH', route('pregnancy-week.update', $id))->attribute('enctype', 'multipart/form-data')->id('insights_validation_form')->open() }}
        @else
             {{ html()->form('POST', route('pregnancy-week.store'))->attribute('enctype','multipart/form-data')->id('insights_validation_form')->open() }}
        @endif
       <div class="row">
           <div class="col-lg-12">
               <div class="card">
                   <div class="card-header d-flex justify-content-between">
                       ...
                       <div class="card-action">
                           <a href="{{ route('pregnancy-week.index') }} " class="btn btn-sm btn-primary" role="button">{{ __('message.back') }}</a>
                       </div>
                   </div>
                       ...
                       <div class="new-user-info">
                           <div class="row">
                               ...
                               <div class="form-group col-md-6">
                                    ...
                                    {{ html()->select('goal_type', ['1' => __('message.track_pragnancy')], old('status'))->class('form-control select2js')->id('goal_type_id') }}
                                </div>

                                <div class="form-group col-md-6">
                                    {{ html()->label(__('message.week') . ' <span class="text-danger"></span>')->class('form-control-label')->attribute('for', 'weeks') }}
                                    {{ html()->select('weeks', isset($id) ? [ $data->weeks => $data->weeks . ' ' . __('message.week') ] : [], old('week'))
                               ...
                               <div class="form-group col-md-6">
                                    {{ html()->label(__('message.view_type') . ' <span class="text-danger">*</span>')->class('form-control-label')->attribute('for', 'view_type_id')  }}
                                    {{ html()->select('view_type', ['' => ''] + getViewType(), $data->type ?? old('type'))
                                                ...
                                                <div class="row">
                                                    @foreach($images as $image)
                                                        ...
                                                        <div class="col-md-1 pr-0 mb-3"> 
                                                            ...
                                                            <a class="text-danger remove-button position-absolute top-0 end-0" href="{{ route('remove.file', ['id' => $image->id, 'type' =>'story_image']) }}"
                                                                data--submit='confirm_form'
                                                                data--confirmation='true'
                                                                data--ajax='true'
                                                                data-toggle='tooltip'
   ...
   @section('bottom_script')
        ...
        $(document).ready(function () {
            ...
            formValidation("#insights_validation_form", {
                ...
                week: { required: "Please select a View Week." },
                    });
                });
        </script>
        <script>
