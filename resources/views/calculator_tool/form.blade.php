<x-master-layout :assets="$assets ?? []">
    <div>
        <?php $id = $id ?? null;?>
        @if(isset($id))
            {{ html()->modelForm($data, 'PATCH', route('calculator-tool.update', $id))->attribute('enctype', 'multipart/form-data')->id('calculator_tool_validation_form')->open() }}
        @else
            {{ html()->form('POST', route('calculator-tool.store'))->attribute('enctype','multipart/form-data')->id('calculator_tool_validation_form')->open() }}  
        @endif
        <div class="row">
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        ...
                        <div class="card-action">
                            <a href="{{ route('calculator-tool.index') }} " class="btn btn-sm btn-primary" role="button">{{ __('message.back') }}</a>
                        </div>
                    </div>
                    ...
                    <div class="card-body">
                        <div class="new-user-info">
                            <div class="row">
                                ...
                                <div class="form-group col-md-6">
                                    {{ html()->label(__('message.blog') . ' <span class="text-danger">*</span>')->class('form-control-label') }}
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <div class="custom-file">
                                        <input type="file" name="calculator_thumbnail_image" class="custom-file-input" accept="image/*">
                                        <label class="custom-file-label">{{  __('message.choose_file',['file' =>  __('message.image') ]) }}</label>
                                    </div>
                                    <span class="selected_file"></span>
                                </div>

                                @if( isset($id) && getMediaFileExit($data, 'calculator_thumbnail_image'))
                                    <div class="col-md-2 mb-2">
                                        <img id="calculator_thumbnail_image_preview" src="{{ getSingleMedia($data,'calculator_thumbnail_image') }}" alt="category-image" class="attachment-image mt-1">
                                        <a class="text-danger remove-file" href="{{ route('remove.file', ['id' => $data->id, 'type' => 'calculator_thumbnail_image']) }}"
        ...
        <script>
            (function ($) {
                $(document).ready(function () {
                    ...
                    formValidation("#calculator_tool_validation_form", {
                        name: { required: true },
                        blog_link: { required: true },
                    }, {
                        name: { required: "Please enter a Name." },
                        blog_link: { required: "Please select a Blog Link."},
                    });
                });
            })(jQuery);
        </script>
